...
async function activate(context) {
    ...
    }), vscode.commands.registerCommand("scaffold.finalizeFile", async (node) => {
    ...
    }), vscode.commands.registerCommand("scaffold.createRevision", async (node) => {
        if (!node || node.isDirectory) {
            return;
        }
        if (node.section === "readyToBuild") {
    ...
    }), vscode.commands.registerCommand("scaffold.markTaskDone", async (node) => {
        if (!node || node.isDirectory) {
            return;
        }
        try {
