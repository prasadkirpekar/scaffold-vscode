...
async function activate(context) {
    ...
    const updateContext = async () => {
        ...
        for (const state of states) {
            await vscode.commands.executeCommand("setContext", `scaffold.section.${state.section}.status`, state.status);
        }
    };
    await updateContext();
    context.subscriptions.push(vscode.commands.registerCommand("scaffold.initializeWorkspace", async () => {
    ...
    }), vscode.commands.registerCommand("scaffold.approveSection", async (section) => {
        if (!section) {
            return;
        }
    ...
    }), vscode.commands.registerCommand("scaffold.approveFile", async (node) => {
        if (!node) {
            return;
        }
        const comment = await vscode.window.showInputBox({
    ...
    }), vscode.commands.registerCommand("scaffold.createFileInSection", async (node) => {
    ...
    }), vscode.commands.registerCommand("scaffold.createFolderInSection", async (node) => {
        if (!node || !node.isDirectory) {
            return;
        }
        ...
        vscode.window.showInformationMessage(outputMode === "both"
        ...
        const confirm = await vscode.window.showWarningMessage(`Are you sure you want to delete "${label}"?`, { modal: true }, "Delete");
        if (confirm !== "Delete") {
            return;
        }
        try {
    ...
    const appendBuildManualLogFromFs = async (uri, msg) => {
    ...
    const contentWatcher = vscode.workspace.createFileSystemWatcher(new vscode.RelativePattern(buildRoot, "**/*"));
    context.subscriptions.push(contentWatcher, contentWatcher.onDidCreate(async (uri) => {
